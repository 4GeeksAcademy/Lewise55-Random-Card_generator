...
window.onload = function() {
  ...
  let fullDeckIndex = Math.floor(Math.random() * fullDeck.length);
  let backIndex = Math.floor(Math.random() * back.length);

  const fullDeck = [];
  const backCard = [];
  for(const suit of ["spades, hearts, diamonds, clubs, joker, back"]){
    for(const card of suit){      
      if(back.includs(card)) {
        backCard.push(card);
      }else{
        fullDeck.push(card);
      }
    }
    document.querySelector(".deck").innerHTML = fullDeck.join;
    console.log("deck");
